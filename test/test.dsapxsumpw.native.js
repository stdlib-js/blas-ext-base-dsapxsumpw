...
var dsapxsumpw = tryRequire( resolve( __dirname, './../lib/dsapxsumpw.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 4', opts, function test( t ) {
	t.strictEqual( dsapxsumpw.length, 4, 'returns expected value' );
	t.end();
});
...
tape( 'the function throws an error if provided a first argument which is not a number', opts, function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dsapxsumpw( value, 5.0, new Float32Array( 10 ), 1 );
		};
	}
});
...
tape( 'the function throws an error if provided a second argument which is not a number', opts, function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dsapxsumpw( 10, value, new Float32Array( 10 ), 1 );
		};
	}
});
...
tape( 'the function throws an error if provided a fourth argument which is not a number', opts, function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dsapxsumpw( 10, 5.0, new Float32Array( 10 ), value );
		};
	}
});
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `0.0`', opts, function test( t ) {
	...
	v = dsapxsumpw( 0, 5.0, x, 1 );
	t.strictEqual( v, 0.0, 'returns expected value' );

	v = dsapxsumpw( -1, 5.0, x, 1 );
	t.strictEqual( v, 0.0, 'returns expected value' );
...
tape( 'if provided a `stride` parameter equal to `0`, the function returns the sum of the first element plus a constant repeated N times', opts, function test( t ) {
	var x;
	var v;

	x = new Float32Array( [ 1.0, -2.0, -4.0, 5.0, 3.0 ] );

	v = dsapxsumpw( x.length, 5.0, x, 0 );
